#ifndef DILEPNALYSIS_H
#define DILEPANALYSIS_H
...
class DiLepAnalysis 
{
	public :
		...
		DiLepAnalysis(TTree *physics, Bool_t tRunningGrid = false, TString tgridFileName = "runningGridMismatch", Bool_t tUseNewGeoData = false);
		~DiLepAnalysis();
	...
	private :
		...
		Bool_t CutFlow2L2L(Double_t weight);
		...
		vector<QuadLepton *> GetQuadLeptonComb(vector<DiLepton *> diMuLepton, vector<DiLepton *> diElLepton); // for 2Mu2El and 2El2Mu
		// Get a single Quad lepton
		DiLepton*  GetDiEvent(vector<DiLepton *> Zcan, Int_t * cutEventPass, Int_t type, TH1D *cutPassHistW, Double_t weight);
		// Cuts on the single Quad lepton
		Bool_t CutDiLepton(DiLepton * Zcan, Int_t * cutEventPass, Int_t type, TH1D *cutPassHistW, Double_t *weightInit);	
		...
		Bool_t CutTrackIso(vector<ChargedLepton *> curr_lep, vector<Double_t> trackPT, Int_t type);
		// Calo Iso
		...
		Bool_t CutCaloIso(vector<ChargedLepton *> curr_lep, vector<Double_t> caloPT, Int_t type);
		// Impact Parameter significance
		Bool_t D0SigCut(vector<ChargedLepton *> curr_lep, Int_t type);
		...
		// Mass calculations 
		void MassCalc (DiLepton * ZCan, Int_t Type);		
		void CorrectFSR(QuadLepton * higgs, Int_t Type);
		void CorrectZMassConstraint(DiLepton * ZCan);
		...
		Double_t getDiWeight(DiLepton * ZCan);
		Double_t getTriggerWeight(DiLepton * ZCan);
		Double_t getLepEffWeight(DiLepton * ZCan);
		void fillZWeight(DiLepton * ZCan);
		
		// CrossSection Weights
		Double_t getMCHiggsMass();
		void fillCrossSection(DiLepton * ZCan);
		...
		void fillTruthRecoMatchedInfo(DiLepton * ZCan);
		...
		Bool_t checkParent(Int_t index, Int_t parentPDGID);
		...
		void fillEventVarInfo(DiLepton * ZCan, Int_t type, Int_t *prodCH);
